...
{\epigraph{\textit{Siempre es buena saber donde esta ubicado uno.}}{}}


\hrule
% #################################################
% #################################################
\section{Costo de Complejidad}\label{sec:cc}
...
El Costo de Complejidad son 2 subconjuntos del TCO: el OPEX natural (visible) y el Overhead del
OPEX (usualmente oculto), causado por fricciones evitables.
\\
\\
Ejemplo:
\begin{itemize}
...
El Costo de Complejidad es el OPEX natural más los gastos ocultos de operar una solución, es decir:
...
Cuatro conceptos que conviene no confundir, porque a menudo comparten la misma palabra:
...
\hline
\textbf{Término} & \textbf{Qué es} \\ \hline
TCO & Costo total de poseer una solución durante todo su ciclo de vida: CAPEX + OPEX + costos indirectos. \\ \hline
OPEX & Gasto recurrente de operación en sentido contable: incluye licencias, infraestructura y servicios, además del trabajo humano. \\ \hline
OPEX natural & La parte del esfuerzo operativo humano que corresponde a fricciones \textit{inevitables}. Es el piso: existe incluso con el mejor diseño. \\ \hline
Overhead del OPEX & La parte del esfuerzo operativo humano que corresponde a fricciones \textit{evitables}. Es lo que Kanso reduce. \\ \hline
Costo de Complejidad & OPEX natural + Overhead del OPEX: todo el esfuerzo operativo humano que exige el alcance medido. Es el denominador del Índice Kanso. \\ \hline
\end{tabular}
\end{table}

El Costo de Complejidad está dentro del TCO y se manifiesta dentro del OPEX, pero no es ninguno de
los dos: el TCO incluye CAPEX, sobre el que ninguna reducción de fricción puede actuar, y el OPEX
incluye gastos no laborales que tampoco. Cuando las fricciones evitables llegan a cero, el Costo de
Complejidad se reduce a su piso, el OPEX natural.
...
% #################################################
% #################################################
\section{La complejidad de las interacciones}

La complejidad de un sistema no depende únicamente de cuántos elementos lo componen, sino de la cantidad, densidad, fragilidad y opacidad de las interacciones entre ellos.

Dos sistemas con el mismo número de herramientas, equipos o procesos pueden tener costos operativos radicalmente distintos si en uno las relaciones entre los elementos son claras, estables y predecibles, mientras que en el otro son ambiguas, frágiles o altamente dependientes de coordinación manual.

En este sentido, el costo de complejidad no surge solo por agregar componentes, sino por el tipo de vínculo que se crea entre ellos.
...
\section{Anclaje cuantitativo de la fricción}\label{sec:anclaje}
...
% #################################################
% #################################################
\section{Índice Kanso}\label{sec:ik}
\begin{quote}
...
\[
IK = \frac{\text{Overhead}}{\text{OPEX}} \times 100
\]

Donde:
\begin{itemize}
    \item \textbf{Overhead} = horas dedicadas a fricciones evitables $\times$ costo hora del rol afectado.
    \item \textbf{OPEX} = horas totales del alcance medido $\times$ costo hora del rol afectado. Este denominador es exactamente el \textit{Costo de Complejidad} del alcance medido (\hyperref[sec:cc]{sección~\ref{sec:cc}}).
\end{itemize}

\textbf{Regla de alcance.} El numerador y el denominador deben cubrir el mismo alcance
---un equipo, un área o la organización completa--- y el mismo periodo. Dos IK calculados sobre
alcances distintos no son comparables entre sí, y esa comparabilidad es justamente lo que sostiene
el seguimiento de tendencias y el benchmarking interno entre equipos.

\textbf{Base de medición.} El IK se calcula sobre el \textit{OPEX operativo} ---el costo del
trabajo humano necesario para operar el sistema durante el periodo medido--- y no sobre el TCO.
El TCO incluye CAPEX y costos de adquisición sobre los que ninguna reducción de fricción puede
actuar; usarlo como denominador diluye el índice y vuelve inaplicables los rangos de
interpretación que siguen.

\textbf{Forma simplificada.} Cuando se usa un único costo hora promedio para todo el alcance, la
tarifa aparece en ambos términos y se cancela, de modo que basta con contar horas:

\[
IK = \frac{\text{horas en fricciones evitables}}{\text{horas totales del alcance}} \times 100
\]

Ambas expresiones son el mismo cálculo. La forma en costo habla el lenguaje del CFO; la forma en
horas es más rápida de obtener para un equipo y es la que se usa en los Quick Start del
Apéndice~\ref{chap:aa}.

Interpretación IK
\begin{itemize}
    \item IK bajo (<15\%) → Sistema eficiente (zona verde). La mayor parte del esfuerzo operativo se dedica a crear valor. Las fricciones inevitables están bajo control.
...
\begin{itemize}
    \item TCO mide el costo total de propiedad (CAPEX + OPEX).
    \item IK mide qué porción del OPEX operativo es desperdicio oculto.
...
4 & Si evitable: lanzar Friction RCA y documentar & Friction RCA \\ \hline
