...
El Índice Kanso (IK) mide qué porcentaje del OPEX operativo se desperdicia en fricciones
evitables. La definición completa, con su regla de alcance, está en la \hyperref[sec:ik]{sección~\ref{sec:ik}}.
...
\begin{enumerate}
    ...
    \item Friction RCA: análisis de causa raíz y contramedidas.
...
\begin{enumerate}
    ...
    Calcula el \textbf{Índice Kanso (IK)} del equipo quincenalmente. Si valoras todas las horas
    a un mismo costo hora promedio, la tarifa se cancela en el numerador y el denominador, así que
    basta con contar horas:
    \[
    IK = \frac{\text{horas perdidas por fricciones evitables}}{\text{horas totales del equipo}} \times 100
    \]  
    Es la forma simplificada de la definición general (\hyperref[sec:ik]{sección~\ref{sec:ik}}), no una
    métrica distinta. Presenta su evolución en la Sprint Review o en el tablero de métricas del equipo.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Friction RCA:} análisis causal colaborativo durante la retrospectiva.
...
\begin{enumerate}
    ...
    Ejemplo:
    \[
    \text{Costo mensual} = 15\,\text{h} \times 50\,\text{USD/h} = 750\,\text{USD}
    \]
    ...
    \]  
    Donde \textit{Overhead} son las horas consumidas por fricciones evitables valoradas a su costo
    hora, y \textit{OPEX} las horas totales del alcance valoradas al mismo costo hora. La escala de
    interpretación y la regla de alcance están en la \hyperref[sec:ik]{sección~\ref{sec:ik}}.
\end{enumerate}
